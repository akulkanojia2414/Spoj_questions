	...
	while(t--)
	{
		...
		for(int i =1;i<=n;i++)
		{
			for(int j=0;j<=w;j++)
			{
				...
				{
					if(j==wt[i-1]) a[j] = min(cost[i-1]+a[j-wt[i-1]],(a[j]==0)?INT_MAX:a[j]);
					else if(a[j-wt[i-1]]==0) a[j] = 0 ;
					else a[j] = min(cost[i-1]+a[j-wt[i-1]],(a[j]==0)?INT_MAX:a[j]);
				}
				//cout<<i<<" "<<j<<" "<<a[j]<<endl;
			}
		}
		if(a[w]==0) cout<<"This is impossible."<<endl;
		else cout<<"The minimum amount of money in the piggy-bank is "<<a[w]<<"."<<endl;
		CLR(a);
	}
}
