...
{
	int test;
	ll x,y,s,t,diff,as,ans;
	double det,ti;
	cin>>test;
	while(test--)
	{
		cin>>x>>y>>s;
		det = sqrtl((7.0*x +5.0*y+2.0*s)*(7.0*x +5.0*y+2.0*s) - 48.0*(x+y)*(s));
		//if(((7.0*x +5.0*y+2.0*s) + det)%(2.0*(x+y))==0)
			ti = ((7.0*x +5.0*y+2.0*s) + det)/(2.0*(x+y));
	//	if(((7.0*x +5.0*y+2.0*s) - det)%(2.0*(x+y))==0) ti = ((7.0*x +5.0*y+2.0*s) - det)/(2.0*(x+y));
		t = llrintl(ti);
		diff = (y-x)/(t-6);
		as = ((t-4)*x - (2*y))/(t-6);
		printf("%d\n",t);
		for(int i =0;i<t;i++) cout<<as + i*diff<<" ";
		cout<<endl;
	}
}
