...
using namespace std;
int best[101][101]={0},csum[101][101]={0};
int main()
{
	int n;
	while(cin>>n)
	{
		...
		for(int i =0;i<n;i++)
			for(int j=i+1;j<n;j++) {if(i==j) csum[i][j] = a[i]; csum[i][j] = (csum[i][j-1]+a[j])%100;
//cout<<csum[i][j]<<" "<<i<<" "<<j<<endl;
}
		int temp,min;
		for(int i =1;i<n;i++)
			for(int j=0;j<n-i;j++){
				...
				for(int  k=j;k<j+i;k++)
					{
						temp = best[j][k] +best[k+1][j+i] + csum[j][k]*csum[k+1][j+i];
						//cout<<i<<" "<<j<<" "<<best[k+1][i+j]<<endl;
						if(temp<min) min = temp;
					}
					//cout<<best[j][i+j]<<" "<<j<<" "<<j+i<<endl;
					best[j][j+i] = min;
			}
